...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..m + 1 {
        c.set(i, 0, 0).expect("Error occurs when lcs gird rows are set to zero.");
    }
    for j in 0..n + 1 {
        c.set(0, j, 0).expect("Error occurs when lcs gird cols are set to zero.");
    }
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
...
fn main() {
    ...
    let lines1 = read_file_lines(filename1)
                                .expect(std::format!("Read file {} error", filename1).as_str());
    let lines2 = read_file_lines(filename2)
                                .expect(std::format!("Read file {} error", filename2).as_str());
