...
fn main() {
    ...
    let user_process = ps_utils::get_target(target)
        .expect(format!("Calling {} failed.", target).as_str());
    ...
    let process_pid = user_process.unwrap().pid;
    let child_processes = ps_utils::get_child_processes(process_pid)
        .expect(format!("Get child processes failed, whose parent is {}", process_pid).as_str());
