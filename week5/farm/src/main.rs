...
fn pull_numbers (remained_numbers: &mut Arc<Mutex<VecDeque<u32>>>) -> Option<u32> {
    let mut remained_numbers_ref = remained_numbers.lock().unwrap();
    remained_numbers_ref.pop_front()
}
...
fn main() {
    ...
    let remained_numbers: Arc<Mutex<VecDeque<u32>>> = Arc::new(Mutex::new(get_input_numbers()));
    ...
    for _ in 0..num_threads {
        ...
        let mut remained_numbers_ref = remained_numbers.clone();
        threads.push(thread::spawn(move || {
            loop {
                if let Some(number) = pull_numbers(&mut remained_numbers_ref) {
